...
function ClassCard({ course, index, role }) {
    ...
    <View style={[s.card, { borderLeftColor: p.accent, borderLeftWidth: 4 }]}>
      <View style={s.cardTop}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <View style={[s.codePill, { backgroundColor: p.accent + '15' }]}>
            <Text style={[s.codeText, { color: p.accent }]}>
              {course.courseCode || '—'}
            </Text>
          </View>
        </View>
      ...
      >
        {filtered.length === 0 ? (
          ...
          <>
            ...
            {filtered.map((c, i) => (
              <ClassCard key={c.id || i} course={c} index={i} role={role} />
            ))}
          </>
        )}
...
const s = StyleSheet.create({
  ...
  codePill:   { borderRadius: 6, paddingHorizontal: 8, paddingVertical: 2, alignSelf: 'flex-start' },
